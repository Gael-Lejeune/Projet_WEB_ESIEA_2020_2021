...
if (isset($_SESSION['cart']) && sizeof($_SESSION['cart'])!=0){
    ...
    $query = $db->prepare('SELECT id_shipment,order_date,estimated_date,address,item_list.amount,name,artist,price,image_url FROM shipment,item_list,item WHERE shipment.id_shipment = item_list.shipment_id AND item.id_item = item_list.item_id AND shipment.id_shipment IN ('.str_repeat('?, ', count($_SESSION['cart']) - 1) . '?)');
    $shipmentList = array();
